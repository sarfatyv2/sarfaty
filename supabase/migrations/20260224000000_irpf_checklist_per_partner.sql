-- Migration: Update get_document_checklist and can_submit_for_analysis
-- to generate one IRPF checklist item per partner per exercise year.
--
-- Before: IRPF was a single hardcoded entry in the base documents VALUES list.
-- After:  IRPF items are generated dynamically from client_authorized_persons
--         CROSS JOIN required exercise years, one item per (partner + year).

-- ============================================================
-- 1. DROP and recreate get_document_checklist
-- ============================================================
...
BEGIN
  ...
  CROSS JOIN (
    SELECT EXTRACT(YEAR FROM NOW())::INTEGER AS exercise_year
    UNION ALL
    SELECT (EXTRACT(YEAR FROM NOW()) - 1)::INTEGER
  ) AS yr
  LEFT JOIN client_documents cd
    ON  cd.client_id      = p_client_id
...
-- ============================================================
-- 2. DROP and recreate can_submit_for_analysis
-- ============================================================
