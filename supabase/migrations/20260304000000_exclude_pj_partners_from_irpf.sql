-- Migration: Exclude PJ (legal entity) partners from IRPF checklist
--
-- IRPF (Imposto de Renda Pessoa Física) applies only to individuals.
-- Partners that are companies (CNPJ — 14 digits) must not generate IRPF items.
-- CPF = 11 digits → individual → IRPF required
-- CNPJ = 14 digits → company → IRPF NOT required
-- NULL cpf → likely a company (PJ partners have no CPF) → IRPF NOT required
