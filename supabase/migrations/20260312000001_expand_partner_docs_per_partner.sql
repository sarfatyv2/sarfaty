-- Migration: Expand partner_id and partner_address_proof into individual per-partner slots
--
-- Previously these were single static checklist items for all partners combined.
-- Now each PF (individual) partner gets their own upload slot, matching the
-- IRPF per-partner pattern. PJ partners (CNPJ) are excluded.
...
BEGIN
  ...
  -- ========================================
  -- 1b. REVENUE — one slot per year (2022–2025)
  -- ========================================
  SELECT
    'revenue'::TEXT,
    ...
    NULL::TEXT,
    yr.ref_year
  FROM (VALUES (2022),(2023),(2024),(2025)) AS yr(ref_year)
  LEFT JOIN client_documents cd
    ON cd.client_id      = p_client_id
