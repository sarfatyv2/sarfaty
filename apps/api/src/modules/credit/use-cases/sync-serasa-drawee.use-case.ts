...
export class SyncSerasaDraweeUseCase {
  ...
  async execute(draweeId: string, rawResponse: unknown): Promise<void> {
    const resp = rawResponse as { reports?: Array<Record<string, unknown>> };
    if (!resp?.reports?.[0]) {
      this.logger.warn(`No report data in Serasa response for drawee ${draweeId}`);
      return;
    }

    const report = resp.reports[0] as { identificationReport?: { address?: { addressLine?: string; zipCode?: string; district?: string; city?: string; state?: string }; phone?: { phoneNumber?: string; areaCode?: string } } };
    ...
    this.logger.log(
      `Enriching drawee ${draweeId} from Serasa: address=${!!addr}, contact=${!!contactPhone}`,
    );
