...
export class NegativeMediaAdapter {
  ...
  async search(
    ...
    const prompt = [
      `Pesquise de forma EXAUSTIVA na internet sobre a empresa "${companyName}" (CNPJ: ${cnpj}${tradeNamePart}).`,
      'Faça MÚLTIPLAS buscas com variações do nome da empresa para cobrir o máximo de resultados.',
      '',
      'Busque por notícias, reportagens, denúncias ou menções dos últimos 10 anos envolvendo:',
      '- Fraude, golpe, estelionato, pirâmide financeira',
      '- Falência, recuperação judicial, concordata',
      '- Trabalho escravo ou análogo, trabalho infantil',
      '- Multas ambientais, crimes ambientais, desmatamento',
      '- Processos criminais, inquéritos policiais, operações policiais (ex: Lava Jato)',
      '- Escândalos, denúncias de corrupção, suborno, propina',
      '- Lavagem de dinheiro, evasão fiscal, sonegação',
      '- Ações coletivas de consumidores, reclamações graves no Procon/Reclame Aqui',
      '- Irregularidades trabalhistas, demissões em massa ilegais',
      '- Sanções administrativas, multas de órgãos reguladores (CVM, BACEN, IBAMA, TCU)',
      '- Disputas societárias graves, acusações entre sócios',
      '',
      ...
      '      "title": "título da notícia ou menção",',
      ...
      '',
      'Regras:',
      '- Seja EXAUSTIVO: inclua TODOS os findings relevantes que encontrar, não pare nos primeiros.',
      ...
      '- HIGH: fraude confirmada, processo criminal, trabalho escravo, lavagem de dinheiro.',
      '- MEDIUM: recuperação judicial, multas ambientais, denúncias em apuração, ações de consumidores.',
      '- LOW: menções negativas antigas (>5 anos) ou de baixa relevância.',
      '- Máximo de 10 findings, priorizando os mais graves e recentes.',
      '- Inclua findings de QUALQUER período dos últimos 10 anos, não apenas recentes.',
    ].join('\n');
