...
const emptyForm: FormData = {
  contactName: '', useType: '', email: '', billingEmail: '', xmlEmail: '',
  phone: '', phoneMobile: '', billingPhone: '', whatsapp: false, isPrimary: false,
};
...
export function DraweeContactsTab({ draweeId }: DraweeContactsTabProps) {
  ...
  function openEdit(c: DraweeContact) {
    setEditingId(c.id);
    setForm({
      contactName: c.contactName ?? '', useType: c.useType ?? '',
      email: c.email ?? '', billingEmail: c.billingEmail ?? '', xmlEmail: c.xmlEmail ?? '',
      phone: c.phone ?? '', phoneMobile: c.phoneMobile ?? '', billingPhone: c.billingPhone ?? '',
      whatsapp: c.whatsapp, isPrimary: c.isPrimary,
    });
    setDialogOpen(true);
  }
  ...
  async function handleSave() {
    setSaving(true);
    try {
      const body = { ...form, contactName: form.contactName || undefined, useType: form.useType || undefined };
      if (editingId) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end">
        <Button size="sm" onClick={openCreate}><Plus size={14} />Adicionar Contato</Button>
      </div>
      {contacts.length === 0 ? (
            ...
            <div key={c.id} className="flex items-center justify-between px-4 py-3">
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-muted-foreground">
                  {[c.email, c.billingEmail && `Cobr: ${c.billingEmail}`, c.phone].filter(Boolean).join(' · ')}
                </p>
              </div>
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2"><Label>Nome</Label><Input value={form.contactName} onChange={(e) => setForm((p) => ({ ...p, contactName: e.target.value }))} /></div>
              ...
              <div className="space-y-2"><Label>Email Cobrança</Label><Input type="email" value={form.billingEmail} onChange={(e) => setForm((p) => ({ ...p, billingEmail: e.target.value }))} /></div>
              <div className="space-y-2"><Label>Email NF-e (XML)</Label><Input type="email" value={form.xmlEmail} onChange={(e) => setForm((p) => ({ ...p, xmlEmail: e.target.value }))} /></div>
              <div className="space-y-2"><Label>Telefone</Label><Input value={form.phone} onChange={(e) => setForm((p) => ({ ...p, phone: e.target.value }))} /></div>
              <div className="space-y-2"><Label>Celular</Label><Input value={form.phoneMobile} onChange={(e) => setForm((p) => ({ ...p, phoneMobile: e.target.value }))} /></div>
              <div className="space-y-2"><Label>Fone Cobrança</Label><Input value={form.billingPhone} onChange={(e) => setForm((p) => ({ ...p, billingPhone: e.target.value }))} /></div>
            </div>
            <div className="flex items-center justify-between"><Label>WhatsApp</Label><Switch checked={form.whatsapp} onCheckedChange={(v) => setForm((p) => ({ ...p, whatsapp: v }))} /></div>
            <div className="flex items-center justify-between"><Label>Contato Principal</Label><Switch checked={form.isPrimary} onCheckedChange={(v) => setForm((p) => ({ ...p, isPrimary: v }))} /></div>
          </div>
          <DialogFooter>
