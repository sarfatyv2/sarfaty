...
function CategoryNode({
  ...
  onDelete: (category: WikiCategory) => void;
  depth?: number;
}) {
  ...
  return (
    <div>
      <div className="flex items-center group/cat pr-1">
        <button
          type="button"
          onClick={() => onSelect(category.id)}
          style={{ paddingLeft: `${8 + depth * 14}px` }}
          className={cn(
            'flex-1 flex items-center gap-2 pr-1 py-1.5 rounded-lg text-[13px] transition-all duration-150 text-left min-w-0',
            isSelected
        ...
        {canEdit && (
          <div className="hidden group-hover/cat:flex items-center flex-shrink-0">
            <button
              type="button"
        ...
        {hasChildren && (
          <button
            ...
            className="w-6 h-6 flex items-center justify-center rounded text-muted-foreground/40 hover:text-muted-foreground transition-colors flex-shrink-0"
          >
            {expanded
...
function CategoryFormDialog({
  ...
  onSaved: () => void;
  editingCategory: WikiCategory | null;
}) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEditing) {
        await api.patch(`/wiki/categories/${editingCategory.id}`, payload);
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { if (!o) handleClose(); }}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader><DialogTitle>{isEditing ? 'Editar Categoria' : 'Nova Categoria'}</DialogTitle></DialogHeader>
...
function DeleteCategoryDialog({ category, onClose, onDeleted }: { category: WikiCategory | null; onClose: () => void; onDeleted: () => void; }) {
  const [deleting, setDeleting] = useState(false);
  const handleDelete = async () => {
    if (!category) return;
    setDeleting(true);
    try {
      await api.delete(`/wiki/categories/${category.id}`);
  ...
  return (
    <Dialog open={!!category} onOpenChange={(o) => { if (!o) onClose(); }}>
      <DialogContent className="sm:max-w-sm">
        ...
        <p className="text-sm text-muted-foreground">Tem certeza que deseja excluir <strong>&quot;{category?.name}&quot;</strong>? Os artigos desta categoria não serão excluídos.</p>
        <DialogFooter>
...
export function WikiLayout({ canEdit }: { canEdit: boolean }) {
  ...
  const [deletingCategory, setDeletingCategory] = useState<WikiCategory | null>(null);
            ...
            {loadingArticle ? (
              ...
              <>
                <div className="mb-8">
                  ...
                  <h1 className="text-3xl font-normal text-foreground tracking-tight leading-tight mb-3">
                    {article.title}
                  </h1>
  ...
  return (
    <div className="flex h-[calc(100vh-8rem)] rounded-xl border border-border/60 bg-card overflow-hidden shadow-sm">
                ...
                <div className="space-y-0.5">
                  {categoryTree.map((category) => (
                    <CategoryNode
                      ...
                      canEdit={canEdit}
                      onEdit={(c) => { setEditingCategory(c); setCategoryDialogOpen(true); }}
      ...
      <CategoryFormDialog
        ...
        categories={categories}
        onSaved={() => { void loadCategories(); void loadArticles(); }}
      ...
      <DeleteCategoryDialog
        ...
        onDeleted={() => {
          if (deletingCategory?.id === selectedCategoryId) setSelectedCategoryId(null);
          void loadCategories();
          void loadArticles();
        }}
      />
